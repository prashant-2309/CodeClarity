...
async def create_release_note(release_note_request: ReleaseNoteRequest):
    ...
    try:
        ...
        if release_note_request.is_first_release:
            ...
            mr_in_release = await get_mrs_between_tags_by_time(
...
async def get_mrs_between_tags_by_time(project_id: int, from_tag: str, to_tag: str) -> set:
    """
    Get MRs merged between tags using timestamps instead of commit comparison.
    This is more reliable than SHA matching.
    """
    try:
        headers = {"PRIVATE-TOKEN": GITLAB_TOKEN}
        ...
        tag_times = {tag['name']: tag['commit']['created_at'] for tag in tags}
        
        from_time = tag_times.get(from_tag)
        to_time = tag_times.get(to_tag)
        
        if not from_time or not to_time:
            print(f"Could not find timestamps for tags: {from_tag}, {to_tag}")
            return set()
